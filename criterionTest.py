...
for i in range(1,10):
    print(i)
    rfc_en = RandomForestRegressor(criterion="entropy",n_estimators=25,random_state=0)
    rfc_gn = RandomForestRegressor(criterion="gini",n_estimators=25,random_state=0)
    rfc_gn = rfc_gn.fit(x_train,y_train)
    score_g=rfc_gn.score(x_test,y_test)
    rfc_en = rfc_en.fit(x_train,y_train)
    score_e=rfc_en.score(x_test,y_test)
    superpa_e.append(score_e)
    superpa_g.append(score_g)
print(max(superpa_e),superpa_e.index(max(superpa_e)))
print(max(superpa_g),superpa_e.index(max(superpa_g)))
plt.figure(figsize=[20,5])
plt.plot(range(1,10),superpa_e,color="yellowgreen", label="entropy", linewidth=2)
plt.plot(range(1,10),superpa_g,color="cornflowerblue", label="gini", linewidth=2)
plt.legend()
plt.show()
